...
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 4)
...
```
```{r}
Result.mid <- Calc.Topologies(LAG = 4)
Result.high <- Calc.Topologies(LAG = 8)
Result.low <- Calc.Topologies(LAG = 0)

plot(x = Result.mid$distance_km, y = Result.mid$TopologyA_EUR.kWh, type = 'b', pch = 19, col = 'orange', ylim = c(0,0.4), las = 1, xaxs = 'i', yaxs = 'i', xlab = "Distance from Shore [km]", ylab = "LCOE [EUR/kWh]")
polygon(x = c(Result.high$distance_km,rev(Result.high$distance_km)),
        y = c(Result.high$TopologyA_EUR.kWh,rev(Result.low$TopologyA_EUR.kWh)), 
        col = rgb(255/255,165/255,0,0.3), border = NA)
```
