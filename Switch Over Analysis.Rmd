...
Topology_A <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, r = 0.052, life = 25, curtailment = 0.05, interconnection = 3, CF.type = 2, ITC = 0, esc = 0.03) { #Follows Van Wingerden et al. (2023)
  ...
  Levelized.Cost <- LCOE(Total.Inv, Tot.OPEX, Net.NRG, r, life, lag = interconnection, CF.type, esc) #EUR/MWh
  Levelized.Cost.CAPEX <- LCOE(Total.Inv, OPEX = 0, ann.energy = Net.NRG, r, life, lag = interconnection, CF.type, esc)
  Levelized.Cost.OPEX <- LCOE(CAPEX = 0, OPEX = Ann.OPEX, ann.energy = Net.NRG, r, life, lag = interconnection, CF.type, esc)
  Levelized.Cost.EL <- LCOE(CAPEX = 0, OPEX = Net.NRG.Cost, ann.energy = Net.NRG, r , life, lag = interconnection, CF.type, esc)
...
Topology_B <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, PEM.price = 1926000, PIPE.price = 615, r = 0.052, PEM.eff = 0.642, life = 25, lag = 3, CF.type = 2, esc = 0.03) { #Follows Van Wingerden et al. (2023)
 ...
 ### Price of Energy (Assume it's just bought from a plant)
  ...
  Levelized.Cost <- LCOE(Total.Inv, Tot.OPEX, Ann.H2.net*1000, r, life, lag, CF.type, esc) #EUR/kgH2
  Levelized.Cost.CAPEX <- LCOE(Total.Inv,OPEX = 0, ann.energy = Ann.H2.net*1000, r, life, lag, CF.type, esc) + LCOH.Desal
  Levelized.Cost.OPEX <- LCOE(CAPEX = 0, OPEX = Ann.OPEX, ann.energy = Ann.H2.net*1000, r, life, lag, CF.type, esc)
  Levelized.Cost.EL <- LCOE(CAPEX = 0, OPEX = Cost.EL, ann.energy = Ann.H2.net*1000, r, life, lag, CF.type, esc)
...
Topology_C <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, PEM.price = 1926000, SHIP.price = 43000000, PEM.eff = 0.642, r = 0.052, life = 25, lag = 3, CF.type = 2, esc = 0.03) { 
  ...
  Levelized.Cost <- LCOE(Total.Inv, Tot.OPEX, Ann.LH2.net*1000, r, life, lag, CF.type, esc) #EUR/kgH2
  Levelized.Cost.CAPEX <- LCOE(Total.Inv,OPEX = 0, ann.energy = Ann.LH2.net*1000, r, life, lag, CF.type, esc) + LCOH.Desal
  Levelized.Cost.SHIP <- LCOE(Ship.Inv/1000000, OPEX = 0, ann.energy = Ann.LH2.net*1000, r, life, lag, CF.type, esc)
  Levelized.Cost.ELPLANT <- LCOE(Ancillary.Inv, OPEX = 0, ann.energy = Ann.LH2.net*1000, r, life, lag, CF.type, esc)
  Levelized.Cost.OPEX <- LCOE(CAPEX = 0, OPEX = Ann.OPEX, ann.energy = Ann.LH2.net*1000, r, life, lag, CF.type, esc)
  Levelized.Cost.EL <- LCOE(CAPEX = 0, OPEX = Cost.EL, ann.energy = Ann.LH2.net*1000, r, life, lag, CF.type, esc)
...
Calc.Topologies <- function(PLANT = 1000, EL.P = 33, CF = 0.5, CURT = 0.05, PEM.P = 1519000, SHIP.P = 43000000, PIPE.P = 615, PEM.EFF = 0.642, DISC = 0.052, LAG = 3, CF.T = 2, CEF = 0.03, Distance = NULL) {
  ...
  for(dist in dist.V) {
    result.A <- Topology_A(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, r = DISC, curtailment = CURT, interconnection = LAG, CF.type = CF.T, esc = CEF)
    result.B <- Topology_B(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, PIPE.price = PIPE.P, PEM.eff = PEM.EFF, r = DISC, lag = LAG, CF.type = CF.T, esc = CEF)
    result.C <- Topology_C(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, SHIP.price = SHIP.P, PEM.eff = PEM.EFF, r = DISC, lag = LAG, CF.type = CF.T, esc = CEF)
...
```{r}
undebug(Topology_A)
Topology_A(dist.km = 20, plant.MW = PLANT, cf = CF, EL.price = EL.P, r = DISC, curtailment = CURT, interconnection = LAG, CF.type = CF.T, esc = CEF)
```

```{r FUNCTION: Run all topologies, only for fixed distances to check difference between topologies}
Calc.Topologies.Stationary <- function(PLANT = 1000, DIST = 20, EL.P = 33, CF = 0.5, CURT = 0.05, PEM.P = 1519000, SHIP.P = 43000000, PIPE.P = 615, PEM.EFF = 0.642, DISC = 0.052, LAG = 3, CF.T = 2, CEF = 0.03) {
...
```{r here}
Calc.Topologies(PLANT = 1000, LAG = 0, Distance = 20, CEF = 0.03)
Calc.Topologies.Stationary(PLANT = 1000, LAG = 9, DIST = 20, CEF = 0.03, EL.P = 35)
Calc.Topologies.Stationary()

Topology_A(dist.km = 20, plant.MW = 1000)

Lag.V <- c(0,3,6,9,12,15,18)
CEF.V <- c(0.03,0.05,0.07)

rows <- list()

for (i in seq_along(Lag.V)) {
  row_vals <- c(Lag = Lag.V[i])
  for (n in seq_along(CEF.V)) {
    lcoe <- Calc.Topologies.Stationary(
      PLANT = 1000,
      DIST  = 20,
      LAG   = Lag.V[i],
      CEF   = CEF.V[n]
    )$TopologyA_EUR.kWh * 1000
    col_name <- paste0("CEF_", CEF.V[n] * 100, "pct")
    row_vals[col_name] <- lcoe
  }
  rows[[i]] <- as.data.frame(t(row_vals))
}
...
# setEPS()
# postscript("Figure3-without.eps")
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Policy Manuscript/Figure3-with.png", width = 2100, height = 1200, res = 300)
par(mar=c(5,5,2,2))
plot(x = Result.A.3$distance_km, y = Result.A.3$TopologyA_EUR.kWh * 100, type = 'b', pch = 19, col = 'orange', ylim = c(0,40), xlim = c(0,2500), las = 1, xaxs = 'i', yaxs = 'i', xlab = "Distance from Shore [km]", ylab = "LCOE [€cent/kWh]")
polygon(x = c(Result.A.18$distance_km,rev(Result.A.18$distance_km)),
        ...
        col = rgb(255/255,165/255,0,0.3), border = 'orange')
polygon(x = c(0,3000,3000,0), y = c(6.6,6.6,12.7,12.7), col = rgb(0/255,204/255,100/255,0.3), border = NA)
abline(h = 6.6, lty = 2, lwd = 2, col = 'black') #low end of offshore wind LCOE according to Lazard (2023)
abline(h = 12.9, lty = 2, lwd = 2, col = 'black') #low end of offshore wind LCOE according to Lazard (2023)
text(2450,10,"Range of OSW LCOE", cex = 1.5, adj = 1)
text(2450,39.6,"18-year lag", cex = 1.2, srt = 38, adj = 1)
text(2450,29.2,"3-year lag", cex = 1.2, srt = 30, adj = 1)
text(2450,23.5,"No lag", cex = 1.2, srt = 28, adj = 1)
...
```{r Visual of only Topology A - alternate}
Result.A.0 <- Calc.Topologies(LAG = 0, PLANT = 1000)
Result.A.3 <- Calc.Topologies(LAG = 3, PLANT = 1000)
Result.A.6 <- Calc.Topologies(LAG = 6, PLANT = 1000)
Result.A.9 <- Calc.Topologies(LAG = 9, PLANT = 1000)
...
# setEPS()
# postscript("Figure3-without.eps")
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Policy Manuscript/Figure3-alternate.png", width = 2100, height = 1200, res = 300)
par(mar = c(5, 5, 2, 5))  # wider right margin for the side band

# 1. Base plot — no data yet, just axes
plot(x    = Result.A.3$distance_km,
     y    = Result.A.3$TopologyA_EUR.kWh * 100,
     type = 'n',                          # draw nothing yet
     pch  = 19, col = 'orange',
     ylim = c(0, 40), xlim = c(0, 2500),
     las = 1, xaxs = 'i', yaxs = 'i',
     xlab = "Distance from Shore [km]",
     ylab = "LCOE [€cent/kWh]")

# 2. Orange lag band (drawn first, so lines sit on top)
polygon(x = c(Result.A.18$distance_km, rev(Result.A.18$distance_km)),
        y = c(Result.A.18$TopologyA_EUR.kWh * 100,
              rev(Result.A.0$TopologyA_EUR.kWh * 100)),
        col = rgb(255/255, 165/255, 0, 0.3), border = 'orange')

# 3. 3-year lag line on top of band
lines(x = Result.A.3$distance_km,
      y = Result.A.3$TopologyA_EUR.kWh * 100,
      type = 'b', pch = 19, col = 'orange')

# 4. Lazard band — placed to the RIGHT of the plot, outside axis area
#    Requires xpd = TRUE to draw outside the plot region
par(xpd = TRUE)
...
polygon(x = c(lazard_x_left,  lazard_x_right,
              lazard_x_right, lazard_x_left),
        y = c(lazard_low,  lazard_low,
              lazard_high, lazard_high),
        col    = rgb(0/255, 204/255, 100/255, 0.4),
        border = 'darkgreen', lwd = 1.2)

# Bracket tick marks at band edges
segments(x0 = lazard_x_left - 10, x1 = lazard_x_right + 10,
         y0 = lazard_low,  y1 = lazard_low,  lwd = 1.5, col = 'darkgreen')
segments(x0 = lazard_x_left - 10, x1 = lazard_x_right + 10,
         y0 = lazard_high, y1 = lazard_high, lwd = 1.5, col = 'darkgreen')

# Label — two lines, centered above the band
text(x = (lazard_x_left + lazard_x_right) / 2,
     y = lazard_high + 3,
     labels = "Lazard\n(2023)",
     cex = 0.85, adj = 0.5, col = 'darkgreen')

par(xpd = FALSE)  # restore clipping

# 5. Existing text labels
text(2450, 39.6, "18-year lag", cex = 1.2, srt = 38, adj = 1)
text(2450, 29.2, "3-year lag",  cex = 1.2, srt = 30, adj = 1)
text(2450, 23.5, "No lag",      cex = 1.2, srt = 28, adj = 1)
...
```{r VISUALIZE Lag}
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Policy Manuscript/Figure10.png", width = 2100, height = 1200, res = 300)

par(mar = c(5,5,2,2))

plot(x = RES.2$lag.years, y = RES.2$LCOE, pch = 15, type = 'b', xlim = c(0,15), ylim = c(60,80),
     xlab = "Years of development", ylab = "LCOE [EUR/MWh]", lwd = 3, cex = 1.2,
     las = 1, xaxs = 'i', yaxs = 'i')
points(x = RES.1$lag.years, y = RES.1$LCOE, pch = 19, type = 'b', col = 'darkblue', lty = 2, lwd = 3, cex = 1.2)
# points(x = RES.2$lag.years, y = RES.2$LCOE, pch = 19, type = 'b', col = 'green')
points(x = RES.3$lag.years, y = RES.3$LCOE, pch = 16, type = 'b', col = 'orange', lty = 3, lwd = 3, cex = 1.2)
points(x = RES.4$lag.years, y = RES.4$LCOE, pch = 17, type = 'b', col = 'darkred', lty = 4, lwd = 3, cex = 1.2)
...
legend('topleft', legend = c("Worst Case", "Levelized Case","Graduated Case","Randomized Case"),
       col = c('darkblue','black','orange','darkred'), pch = c(19,15,16,17), lty = c(2,1,3,4), cex = 1)
...
(Baseline <- Topology_A(dist.km = 20, 
                       ...
                       curtailment = 0.05, 
                       interconnection = 10, 
                       CF.type = 2)$LCOE.tot) #in EUR/MWh. 65.8
...
Policy.ITC <- Topology_A(dist.km = 20, 
                         ...
                         interconnection = 10, 
                         CF.type = 2,
                         ITC = 0.04)$LCOE.tot #64.81 EUR/MWh 
...
Policy.LAG <- Topology_A(dist.km = 20, 
                         ...
                         interconnection = 9, 
                         CF.type = 2,
                         ITC = 0)$LCOE.tot #64.82 EUR/MWh. What will it cost to reduce permitting time by 1 year?
...
```{r Figure 4}
# Figure4
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, PEM.EFF = 0.641, LAG = 0)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, PEM.EFF = 0.642, LAG = 0)
Result.low <- Calc.Topologies(EL.P = 29, PEM.P = 1198000, PEM.EFF = 0.643, LAG = 0)
...
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Environmental Research Letters/Figures/Figure4.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
plot(x = Result.mid$distance_km, y = Result.mid$TopologyA_EUR.MJ, type = 'n', pch = 19, col = 'orange', ylim = c(0,0.16), xlim = c(0,2500), las = 1, xaxs = 'i', yaxs = 'i', yaxt = 'n', xlab = "Distance from Shore [km]", ylab = "")
...
polygon(x = c(Result.high$distance_km,rev(Result.high$distance_km)),
        ...
        col = rgb(255/255,165/255,0,0.5), border = 'orange')
lines(x = Result.mid$distance_km, y = Result.mid$TopologyA_EUR.MJ, type = 'b', pch = 19, col = 'orange')
abline(v = 370, col = 'red')
text(375,0.10,"EEZ", adj = c(1,1),srt = 90)
...
mtext("€cent/MJ", side = 2, line = 1.75)
mtext("€cent/kWh", side = 2, line = 5.9, col = 'orange')
mtext("€/kgH2", side = 2, line = 9.9, col = 'blue')
...
# data.frame(Result.high$distance_km, Result.high$TopologyA_EUR.MJ - Result.low$TopologyB_EUR.MJ)

```

```{r Figure 5}
# Figure5
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, PEM.EFF = 0.641, LAG = 20)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, PEM.EFF = 0.642, LAG = 3)
Result.low <- Calc.Topologies(EL.P = 29, PEM.P = 1198000, PEM.EFF = 0.643, LAG = 0)
...
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Environmental Research Letters/Figures/Figure5.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
plot(x = Result.mid$distance_km, y = Result.mid$TopologyA_EUR.MJ, type = 'n', pch = 19, col = 'orange', ylim = c(0,0.16), xlim = c(0,2500), las = 1, xaxs = 'i', yaxs = 'i', yaxt = 'n', xlab = "Distance from Shore [km]", ylab = "")
lines(x = Result.mid$distance_km, y = Result.mid$TopologyB_EUR.MJ, type = 'b', pch = 17, col = 'blue')
lines(x = Result.mid$distance_km, y = Result.mid$TopologyC_EUR.MJ, type = 'b', pch = 15, col = 'blue')
...
# legend('bottomright', legend = c("A: HVDC","C: H2 Ships"),
#        pch = c(19,15), col = c("orange","blue"), lty = 1)
text(2470,0.116,"20-year lag", cex = 1.2, srt = 38, adj = 1)
text(2470,0.082,"3-year lag", cex = 1.2, srt = 29, adj = 1)
text(2470,0.061,"No lag", cex = 1.2, srt = 21, adj = 1)
...
# data.frame(Result.high$distance_km, Result.high$TopologyA_EUR.MJ - Result.mid$TopologyB_EUR.MJ)

```

```{r Figure 6}
#Figure6
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, PEM.EFF = 0.641, LAG = 20, CURT = 0.35)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, PEM.EFF = 0.642, LAG = 3, CURT = 0.1)
Result.low <- Calc.Topologies(EL.P = 29, PEM.P = 1198000, PEM.EFF = 0.643, LAG = 0, CURT = 0.05)
...
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Environmental Research Letters/Figures/Figure6.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
plot(x = Result.mid$distance_km, y = Result.mid$TopologyA_EUR.MJ, type = 'n', pch = 19, col = 'orange', ylim = c(0,0.16), xlim = c(0,2500), las = 1, xaxs = 'i', yaxs = 'i', yaxt = 'n', xlab = "Distance from Shore [km]", ylab = "")
lines(x = Result.mid$distance_km, y = Result.mid$TopologyB_EUR.MJ, type = 'b', pch = 17, col = 'blue')
lines(x = Result.mid$distance_km, y = Result.mid$TopologyC_EUR.MJ, type = 'b', pch = 15, col = 'blue')
...
# legend('bottomright', legend = c("A: HVDC","C: H2 Ships"),
#        pch = c(19,15), col = c("orange","blue"), lty = 1)
text(2340,0.157,"35% Curtailment", cex = 1.2, srt = 44, adj = 1)
text(2470,0.089,"10% Curtailment", cex = 1.2, srt = 28, adj = 1)
text(2470,0.061,"5% Curtailment", cex = 1.2, srt = 20, adj = 1)
...
```{r Figure X - Profitability}
# Figure6
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, LAG = 20, CURT = 0.35)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 3, CURT = 0.1)
Result.low <- Calc.Topologies(EL.P = 29, PEM.P = 1198000, LAG = 0, CURT = 0.05)

#Figure6. best for H2
Result.high <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 14, CURT = 0.35)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 7, CURT = 0.1)
Result.low <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 0, CURT = 0.05)

# plot(x = Result.DF$distance_km, y = Result.DF$TopologyB_EUR.kgH2, type = 'b', pch = 19, col = 'blue', ylim = c(0,10))
# lines(x = Result.DF$distance_km, y = Result.DF$TopologyC_EUR.kgLH2, type = 'b', pch = 19, col = 'green')
# abline(v = 200, col = 'red')
MJ.kWh <- 3.6 #energy content of electricity. 3.6 MJ/kWh
MJ.kgH2 <- 120 #energy content of hydrogen. 120 MJ/kgH2

EUR.kWh <- 0.10 #energy price of electricity in EUR/kWh
EUR.kgH2 <- 7 #energy price of hydrogen in EUR/kgHw

EL.EUR.MJ <- EUR.kWh/MJ.kWh #energy price in EUR/MJ for electricity
H2.EUR.MJ <- EUR.kgH2/MJ.kgH2 #energy price in EUR/MJ for hydrogen.

axs.ticks <- c(-0.06,-0.04,-0.02,0,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16)

# png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Conversion and Management Manuscript/Figure6.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
plot(x = Result.mid$distance_km, y = EL.EUR.MJ - Result.mid$TopologyA_EUR.MJ, type = 'n', pch = 19, col = 'orange', ylim = c(-0.06,0.16), xlim = c(0,2500), las = 1, xaxs = 'i', yaxs = 'i', yaxt = 'n', xlab = "Distance from Shore [km]", ylab = "")
lines(x = Result.mid$distance_km, y = H2.EUR.MJ - Result.mid$TopologyB_EUR.MJ, type = 'b', pch = 17, col = 'blue')
lines(x = Result.mid$distance_km, y = H2.EUR.MJ - Result.mid$TopologyC_EUR.MJ, type = 'b', pch = 15, col = 'blue')
# abline(h = 6.6/MJ.kWh, lty = 2, col = 'black') #low end of offshore wind LCOE according to Lazard (2023)
# abline(h = 12.9/MJ.kWh, lty = 2, col = 'black') #low end of offshore wind LCOE according to Lazard (2023)
# lines(x = Result.high$distance_km, y = Result.high$TopologyA_EUR.MJ, type = 'l', pch = 15, col = 'orange')

polygon(x = c(Result.high$distance_km,rev(Result.high$distance_km)),
        y = c(H2.EUR.MJ - Result.high$TopologyB_EUR.MJ,rev(H2.EUR.MJ - Result.low$TopologyB_EUR.MJ)),
        col = rgb(0,0,1,0.3), border = 'blue')
polygon(x = c(Result.high$distance_km,rev(Result.high$distance_km)),
        y = c(H2.EUR.MJ - Result.high$TopologyC_EUR.MJ,rev(H2.EUR.MJ - Result.low$TopologyC_EUR.MJ)), 
        col = rgb(0,0,1,0.3), border = 'blue')
polygon(x = c(Result.high$distance_km,rev(Result.high$distance_km)),
        y = c(EL.EUR.MJ - Result.high$TopologyA_EUR.MJ,rev(EL.EUR.MJ - Result.low$TopologyA_EUR.MJ)), 
        col = rgb(255/255,165/255,0,0.5), border = 'orange')
lines(x = Result.mid$distance_km, y = EL.EUR.MJ - Result.mid$TopologyA_EUR.MJ, type = 'b', pch = 19, col = 'orange')
abline(v = 370, col = 'red')
text(375,0.10,"EEZ", adj = c(1,1),srt = 90)
axis(side = 2, pos = 0, las = 1, at = axs.ticks, labels = axs.ticks*100) #EURcent/MJ
axis(side = 2, pos = -375, las = 1, at = axs.ticks, labels = axs.ticks*MJ.kWh*100, col = 'orange', col.axis = 'orange') #EURcent/kWh
...
# legend('bottomright', legend = c("A: HVDC","C: H2 Ships"),
# #        pch = c(19,15), col = c("orange","blue"), lty = 1)
# text(2340,0.157,"35% curtail", cex = 1.2, srt = 45, adj = 1)
# text(2470,0.089,"10% curtail", cex = 1.2, srt = 29, adj = 1)
# text(2470,0.061,"5% curtail", cex = 1.2, srt = 21, adj = 1)
...
```{r Country H2 and EL prices}
usd_to_eur <- 0.91


# price_data <- data.frame(
#   Country = c("USA", "Germany", "France", "UK", "Japan", "China", "India", "Australia", "Canada", "UAE"),
#   Elec_Low = usd_to_eur * c(0.13, 0.34, 0.20, 0.32, 0.20, 0.06, 0.05, 0.18, 0.08, 0.03)/MJ.kWh, #now in EUR/MJ
#   Elec_High = usd_to_eur * c(0.19, 0.42, 0.30, 0.40, 0.27, 0.10, 0.09, 0.45, 0.19, 0.07)/MJ.kWh,
#   H2_Low = usd_to_eur * c(3.5, 7.0, 6.5, 6.5, 5.0, 2.0, 3.5, 4.0, 4.5, 1.5)/MJ.kgH2,
#   H2_High = usd_to_eur * c(6.0, 10.5, 9.5, 9.0, 7.0, 4.5, 5.0, 7.0, 6.5, 3.5)/MJ.kgH2
# )

# price_data <- data.frame(
#   Country = c("USA", "Japan", "Germany", "China", "Netherlands", "UK"),
#   Elec_Low = usd_to_eur * c(0.083, 0.20, 0.34, 0.075, 0.27, 0.32)/MJ.kWh,
#   Elec_High = usd_to_eur * c(0.1747, 0.27, 0.42, 0.093, 0.32, 0.4)/MJ.kWh,
#   H2_Low = usd_to_eur * c(2, 3, 1, 4, 5, 13)/MJ.kgH2,
#   H2_High = usd_to_eur * c(23, 23, 15, 10, 12, 20)/MJ.kgH2
# )

# price_data <- data.frame(
#   Country = c("Denmark","Germany","Netherlands","Portugal","UK","USA","China","EU-Average"),
#   Elec_Low = c(0.1485,0.1596,0.1493,0.1164,0.27,0.1243,0.075,0.1934)/MJ.kWh,
#   Elec_High = c(0.3636,0.2973,0.4436,0.2983,0.5,0.1499,0.093,0.2401)/MJ.kWh,
#   H2_Low = c(3,3,3,3,3,2,6,3)/MJ.kgH2,
#   H2_High = c(10,10,10,10,10,10,8,10)/MJ.kgH2
# )

price_data <- data.frame(
  Country = c("UK","USA","China","EU-Average"),
  Elec_Low = c(0.27,0.1243,0.075,0.1934)/MJ.kWh,
  Elec_High = c(0.5,0.1499,0.093,0.2401)/MJ.kWh,
  H2_Low = c(3,2,6,3)/MJ.kgH2,
  H2_High = c(10,10,8,10)/MJ.kgH2
)

# Calculate centers and radii for ellipses
data <- price_data %>%
  mutate(
    x_center = (Elec_Low + Elec_High) / 2,
    y_center = (H2_Low + H2_High) / 2,
    x_radius = (Elec_High - Elec_Low) / 2,
    y_radius = (H2_High - H2_Low) / 2
  )

# Generate ellipse points
get_ellipse <- function(cx, cy, rx, ry, n = 100){
...
# Build ellipse data for plotting
ellipse_data <- do.call(rbind, lapply(1:nrow(data), function(i) {
  e <- get_ellipse(data$x_center[i], data$y_center[i], data$x_radius[i], data$y_radius[i])
  e$Country <- data$Country[i]
  return(e)
}))

ellipses_layer <- geom_polygon(
  data = ellipse_data,
  aes(x = x, y = y, group = Country, fill = Country),
  alpha = 0.5, color = "black"
)

labels_layer <- geom_text(
  data = data,
  aes(x = x_center, y = y_center, label = Country),
  size = 3
)

# Plot
ggplot(ellipse_data, aes(x = x, y = y)) +
  geom_polygon(aes(fill = Country, group = Country), alpha = 0.5, color = "black") +
  geom_text(data = data, aes(x = x_center, y = y_center, label = Country), size = 3) +
  labs(
    title = "Hydrogen vs Electricity Residential Price Ranges",
    x = "Electricity Price (EUR/MJ)",
    y = "Hydrogen Price (EUR/MJ)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```

#Profitability Heat Map Base
```{r Profitability Heat Map}
Dist <- 1000 #375 #km distance of the EEZ

High_Topologies <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 14, CURT = 0.35, Distance = Dist)
Mid_Topologies <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 7, CURT = 0.1, Distance = Dist)
Low_Topologies <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 0, CURT = 0.05, Distance = Dist)

H2_prices <- seq(0,12,by = 1) #prices in EUR/kgH2
H2_prices_breaks <- seq(0, 12, by = 2)
EL_prices <- seq(0, 0.5, by = 0.025) #prices in EUR/kWh
EL_prices_breaks <- seq(0, 0.5, by = 0.1)

H2_prices_MJ <- H2_prices/MJ.kgH2 #energy price in EUR/MJ for hydrogen.
EL_prices_MJ <- EL_prices/MJ.kWh #energy price in EUR/MJ for electricity.

rm(Heatmap_DF)

for(HP in H2_prices_MJ){
  for(EP in EL_prices_MJ){
    #Calculate profit of Topology A, B, and C
    TOPA_Profit <- EP - Low_Topologies$TopologyA_EUR.MJ
    TOPB_Profit <- HP - Low_Topologies$TopologyB_EUR.MJ
    TOPC_Profit <- HP - Low_Topologies$TopologyC_EUR.MJ
    
    #Calculate profit difference of A and B, and that between A and C
    TOPB_TOPA <- TOPB_Profit - TOPA_Profit
    TOPC_TOPA <- TOPB_Profit - TOPA_Profit
    
    #Record to a new data frame
    if(!exists('Heatmap_DF')) {
      Heatmap_DF <- data.frame(EL_Price = EP, H2_Price = HP,
                               TOPA_Positive = (TOPA_Profit > 0),
                               TOPB_Positive = (TOPB_Profit > 0),
                               TOPC_Positive = (TOPC_Profit > 0),
                               B_A_Difference = TOPB_TOPA,
                               C_A_Difference = TOPC_TOPA)
    } else {
      new.row <- data.frame(EL_Price = EP, H2_Price = HP,
                            TOPA_Positive = (TOPA_Profit > 0),
                            TOPB_Positive = (TOPB_Profit > 0),
                            TOPC_Positive = (TOPC_Profit > 0),
                            B_A_Difference = TOPB_TOPA,
                            C_A_Difference = TOPC_TOPA)
      Heatmap_DF <- rbind(Heatmap_DF, new.row)
    }
...
```{r}
pct_pth_base <- sum(Heatmap_DF$B_A_Difference > 0)/dim(Heatmap_DF)[1]
pct_hvdc_base <- sum(Heatmap_DF$B_A_Difference < 0)/dim(Heatmap_DF)[1]
pct_pth_base
pct_hvdc_base
```


```{r Heatmap of Top B to Top A Difference}
p <- ggplot() + 
  geom_tile(data = Heatmap_DF, aes(x = EL_Price, y = H2_Price, fill = B_A_Difference)) +
  scale_fill_gradient2(
    low = "orange",       # for negative values
    mid = "white",     # for zero
    high = "blue",     # for positive values
    breaks = seq(-0.151, 0.151, by = 0.05), #positions of the breaks for color gradient changes
    limits = c(-0.151, 0.151),
    midpoint = 0,
    labels = c("HVDC more profitable", "-10", "-5", "0", "5", "10", "PtH more profitable"),
    name = "Profit difference (€cent/MJ)"
  ) +
  # scale_x_continuous(labels = function(x) round(x*MJ.kWh,3)) + 
  scale_x_continuous(breaks = EL_prices_breaks / MJ.kWh,
                     labels = EL_prices_breaks) + 
  # scale_y_continuous(labels = function(y) round(y*MJ.kgH2,5)) + 
  scale_y_continuous(breaks = H2_prices_breaks / MJ.kgH2,
                     labels = H2_prices_breaks) + 
  labs(
    # title = "Profitability Difference: Topology B and A",
  ...
  annotate("text",
    x = 0, y = 12/MJ.kgH2, # top-left corner in data coordinates
    label = "PtH more profitable\nin 33% of scenarios",
    hjust = 0, vjust = 1,
    size = 4, fontface = "bold",
    color = "black"
  ) +
  annotate("text",
    x = 0.5/MJ.kWh, y = 0, # bottom-right corner in data coordinates
    label = "HVDC more profitable\nin 67% of scenarios",
    hjust = 1, vjust = 0,
    size = 4, fontface = "bold",
    color = "black"
  ) +
...
for(name in unique(ellipse_data$Country)) {
  p <- p + geom_polygon(data = ellipse_data[ellipse_data$Country == name, ], aes(x = x, y = y), alpha = 0.1, color = "black")
}

p <- p + coord_cartesian(clip = "on") + geom_text(data = data, aes(x = x_center, y = y_center, label = Country), size = 3)

png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Conversion and Management Manuscript/Figure11_1000base.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
p
dev.off()
...
```{r Heatmap of B to C 2}
p <- ggplot(Heatmap_DF, aes(x = EL_Price, y = H2_Price)) +
  geom_tile(aes(fill = B_A_Difference)) +
  scale_fill_gradient2(
    low = "red",
    mid = "white",
    high = "blue",
...
# Overlay black tiles where either TOPA_Positive or TOPB_Positive is FALSE
p <- p + 
  geom_tile(data = Heatmap_DF %>% filter(!TOPA_Positive & TOPB_Positive), fill = "gray") +
  geom_tile(data = Heatmap_DF %>% filter(TOPA_Positive & !TOPB_Positive), fill = "yellow") +
  geom_tile(data = Heatmap_DF %>% filter(!TOPA_Positive & !TOPB_Positive), fill = "black")

# Add labels and theme
p <- p +
  theme_minimal() +
  labs(
    title = "Heatmap of B_A_Difference by EL_Price and H2_price",
    x = "EL Price",
    y = "H2 Price"
  ) +
  theme(
...
for(HP in H2_prices_MJ){
  for(EP in EL_prices_MJ){
    #Calculate profit of Topology A, B, and C
    TOPA_Profit <- EP - High_Topologies$TopologyA_EUR.MJ
    TOPB_Profit <- HP - Mid_Topologies$TopologyB_EUR.MJ
    TOPC_Profit <- HP - Mid_Topologies$TopologyC_EUR.MJ
...
p <- ggplot() + 
  ...
  scale_fill_gradient2(
    ...
    mid = "white",     # for zero
    high = "blue",     # for positive values
    breaks = seq(-0.15, 0.15, by = 0.05), #positions of the breaks for color gradient changes
    limits = c(-0.15, 0.15),
    midpoint = 0,
    labels = c("HVDC more profitable", "-10", "-5", "0", "5", "10", "PtH more profitable"),
    name = "Profit difference (€cent/MJ)"
...
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Conversion and Management Manuscript/Figure11_1000alt.png", width = 2100, height = 1200, res = 300)
par(mar = c(5,12,1,1))
p
dev.off()
...
```{r Heatmap of Top C to Top A Difference}
p <- ggplot() +
  geom_tile(data = Heatmap_DF, aes(x = EL_Price, y = H2_Price, fill = C_A_Difference)) +
  scale_fill_gradient2(
    low = "orange",       # for negative values
    mid = "white",     # for zero
    high = "blue",     # for positive values
    midpoint = 0,
    name = "Profit difference (€/MJ)"
  ) +
    # scale_x_continuous(labels = function(x) round(x*MJ.kWh,3)) + 
  scale_x_continuous(breaks = EL_prices_breaks / MJ.kWh,
                     labels = EL_prices_breaks) + 
  ...
  scale_y_continuous(breaks = H2_prices_breaks / MJ.kgH2,
                     labels = H2_prices_breaks) +
  labs(
    title = "Profitability Differences beetween topology C and A",
    x = "Electricity Price (€/MJ)",
    y = "Hydrogen Price (€/MJ)"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.title = element_text(face = "bold")
      ...
      annotate(
    "text",
    x = 0, y = 10/MJ.kgH2, # top-left corner in data coordinates
    label = "Topology C\nmore profitable",
    hjust = 0, vjust = 1,
    size = 4, fontface = "bold",
    color = "black"
...
for(name in unique(ellipse_data$Country)) {
  p <- p + geom_polygon(data = ellipse_data[ellipse_data$Country == name, ], aes(x = x, y = y), alpha = 0.25, color = "black")
}

p <- p + geom_text(data = data, aes(x = x_center, y = y_center, label = Country), size = 3)

p

```

```{r Heatmap of C to A 2}
p <- ggplot(Heatmap_DF, aes(x = EL_Price, y = H2_Price)) +
  geom_tile(aes(fill = C_A_Difference)) +
  scale_fill_gradient2(
    low = "red",
    mid = "white",
    high = "blue",
...
# Overlay black tiles where either TOPA_Positive or TOPB_Positive is FALSE
p <- p + 
  geom_tile(data = Heatmap_DF %>% filter(!TOPA_Positive & TOPC_Positive), fill = "gray") +
  geom_tile(data = Heatmap_DF %>% filter(TOPA_Positive & !TOPC_Positive), fill = "yellow") +
  geom_tile(data = Heatmap_DF %>% filter(!TOPA_Positive & !TOPC_Positive), fill = "black")

# Add labels and theme
p <- p +
  theme_minimal() +
  labs(
    title = "Heatmap of C_A_Difference by EL_Price and H2_price",
    x = "EL Price",
    y = "H2 Price"
  ) +
  theme(
...
# Create a dataframe
df <- data.frame(
  Parameter = c("Discount Rate [%]", "Years of lag [years]","Annual CAPEX Escalation [%]","Distance to shore [km]","Electricity cost [€/MWh]","Project Size [MW]"),
  Low_Input = c(-10.48,-6.97,-3.21,-1.7,-5.54,0.01),
  Low_Input_Label = c(55,59,63,64,60,66),
  High_Input = c(21.12,7.98,8.34,5.99,1.07,-1.19),
  High_Input_Label = c(87,74,74,72,67,65)
)

# Reshape data for ggplot
df_long <- melt(df, id.vars = "Parameter", 
                measure.vars = c("Low_Input", "High_Input"),
                variable.name = "Input_Type", value.name = "Impact")

# Ensure correct factor order (maintains given order)
df_long$Parameter <- factor(df_long$Parameter, levels = rev(df$Parameter))

# Assign colors and patterns
df_long$Fill <- ifelse(df_long$Input_Type == "Low_Input", "Low Input", "High Input")  # Orange for Low, White for High
df_long$Pattern <- ifelse(df_long$Input_Type == "Low_Input", "none", "stripe")  # Stripes only for High Input
df_long$Pattern_Color <- ifelse(df_long$Input_Type == "Low_Input", "orange", "darkblue")  # Blue stripes for High

# Add corresponding labels
df_long$Label <- c(df$Low_Input_Label, df$High_Input_Label)

# Create tornado plot
png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Environmental Research Letters/Figures/Figure7.png", width = 2100, height = 1200, res = 300)

par(mar=c(5,5,2,2))

ggplot(df_long, aes(x = Impact, y = Parameter, fill = Fill, 
                     pattern = Pattern, pattern_color = Pattern_Color)) +
  geom_bar_pattern(stat = "identity", width = 0.5,
                   pattern_fill = "darkblue", pattern_colour = "darkblue", pattern_density = 0.5,
                   color = "black", size = 1) +  # Black border around all bars
  scale_fill_manual(values = c("Low Input" = "orange", "High Input" = "white")) +  # Orange for Low, White for High
  scale_pattern_manual(values = c("none", "stripe")) +  # Stripes only for High Input
  scale_pattern_color_manual(values = c("orange" = "orange", "darkblue" = "darkblue")) +  # Blue stripes for High Input
  geom_text(aes(label = Label, x = Impact + ifelse(Impact > 0, 2, -2)), 
            hjust = ifelse(df_long$Impact > 0, 0, 1), size = 5) + 
  geom_vline(xintercept = 0, color = "darkred", linewidth = 1.2) +
  xlim(-15,25) + 
  labs(title = "", x = "Change from Baseline LCOE (€/MWh)", y = "") +
  annotate("text", x = -0.4, y = 6.5, label = "Baseline LCOE", size = 4, hjust = 1, color = "darkred") +
  annotate("text", x = 0.4, y = 6.5, label = "= 65.8 €/MWh", size = 4, hjust = 0, color = "darkred") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(-20, 30, 5), limits = c(-20, 30), expand = c(0,0)) +
  theme(legend.position = "right", 
        axis.text.y = element_text(size = 12, color = "black"),
        axis.text.x = element_text(size = 12, color = "black"),
        axis.line.x = element_line(color = "black", size = 1),  # Full black x-axis line
        axis.ticks.x = element_line(color = "black", size = 1),  # Black tick marks on x-axis
        panel.grid.major = element_blank(),  # Remove major grid lines
        panel.grid.minor = element_blank(),  # Remove minor grid lines
        panel.background = element_blank(),  # Remove background panel color
        plot.background = element_blank(),  # Remove background around the plot
        plot.margin = margin(10,10,20,10)) +
  guides(fill = guide_legend(title = "Parameter Type",
                             override.aes = list(
                               fill = c("white","orange"),
                               pattern = c("stripe","none"),
                               pattern_fill = c("darkblue",NA),
                               pattern_color = c("darkblue",NA),
                               color = "black")),
         pattern = "none") 
...
```{r Spider Plot A vs. B}

elc_cost_x <- c(-12.12,0,6.06)
elc_cost_y <- c(-7.11,0,0.40)

elcz_eff_x <- c(-3.427,0,9.034)
elcz_eff_y <- c(3.557,0,-15.415)

pipe_cost_x <- c(-18.699,0,13.821)
pipe_cost_y <- c(0,0,1)

elcz_cost_x <- c(-21.132,0,26.794)
elcz_cost_y <- c(-18.972,0,18.972)

data <- data.frame(
  Parameter = rep(c("elc_cost", "elcz_eff", "pipe_cost", "elcz_cost"), each = 3),
  Input_Change = c(elc_cost_x, elcz_eff_x, pipe_cost_x, elcz_cost_x),
  Outcome_Change = c(elc_cost_y, elcz_eff_y, pipe_cost_y, elcz_cost_y)
)

png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Policy Manuscript/Figure8a.png", width = 1400, height = 1200, res = 300)

par(mar=c(5,5,2,2))

ggplot(data, aes(x = Input_Change, y = Outcome_Change, group = Parameter, color = Parameter, linetype = Parameter)) +
  geom_line(size = 1.2) +  # Draw lines connecting the points
  geom_point(size = 3) +   # Add points at each (x, y) pair
  coord_cartesian(xlim = c(-50, 50), ylim = c(-50, 50)) +  # Set x and y limits
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 12, color = "black"),  
    axis.text.y = element_text(size = 12, color = "black"),  
  ...
  labs(
    title = "",
    x = "Input Parameter (% of baseline)",
    y = "LCOE difference (% of baseline)"
  ) +
  geom_hline(yintercept = 0, linetype = "solid", color = "black", size = 0.5) +  
  geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 0.5) +  
  scale_x_continuous(breaks = seq(-50, 50, 10), limits = c(-50, 50), expand = c(0, 0)) +  
  scale_y_continuous(breaks = seq(-50, 50, 10), limits = c(-50, 50), expand = c(0, 0)) +  
  scale_color_manual(
    values = c("elc_cost" = "orange", "elcz_eff" = "darkblue", "pipe_cost" = "darkgreen", "elcz_cost" = "darkred"),
    labels = c("elc_cost" = "Electricity Cost (€/MWh)", "elcz_eff" = "Electrolyzer Efficiency (%)", "pipe_cost" = "Pipeline Cost (€/MW)", "elcz_cost" = "Electrolyzer Cost (€/MW)")  # Editable legend labels
  ) +  
  scale_linetype_manual(
    values = c("elc_cost" = "solid", "elcz_eff" = "dashed", "pipe_cost" = "twodash", "elcz_cost" = "dotted"),
    labels = c("elc_cost" = "Electricity Cost (€/MWh)", "elcz_eff" = "Electrolyzer Efficiency (%)", "pipe_cost" = "Pipeline Cost (€/MW)", "elcz_cost" = "Electrolyzer Cost (€/MW)")) +
...
```{r Spider Plot A vs. C}

elc_cost_x <- c(-12.121,0,6.061)
elc_cost_y <- c(-7.115,0,0.395)

elcz_eff_x <- c(-3.427,0,9.034)
elcz_eff_y <- c(3.557,0,-15.415)

ship_cost_x <- c(-41.860,0,86.047)
ship_cost_y <- c(-3.077,0,0)

elcz_cost_x <- c(-21.132,0,26.794)
elcz_cost_y <- c(-18.972,0,18.972)

data <- data.frame(
  Parameter = rep(c("elc_cost", "elcz_eff", "ship_cost", "elcz_cost"), each = 3),
  Input_Change = c(elc_cost_x, elcz_eff_x, ship_cost_x, elcz_cost_x),
  Outcome_Change = c(elc_cost_y, elcz_eff_y, ship_cost_y, elcz_cost_y)
)

png(filename = "C:/Users/Rudolph/Desktop/Rudolph/0 - CMU - Portugal PhD Files/0 - Research/Transmission Switch Over/Energy Policy Manuscript/Figure8b.png", width = 1400, height = 1200, res = 300)
...
ggplot(data, aes(x = Input_Change, y = Outcome_Change, group = Parameter, color = Parameter, linetype = Parameter)) +
  ...
  geom_hline(yintercept = 0, linetype = "solid", color = "black", size = 0.5) +  
  geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 0.5) +  
  scale_x_continuous(breaks = seq(-50, 50, 10), limits = c(-100, 100), expand = c(0, 0)) +  
  scale_y_continuous(breaks = seq(-50, 50, 10), limits = c(-50, 50), expand = c(0, 0)) +  
  scale_color_manual(
    values = c("elc_cost" = "orange", "elcz_eff" = "darkblue", "ship_cost" = "magenta", "elcz_cost" = "darkred"),
    labels = c("elc_cost" = "Electricity Cost (€/MWh)", "elcz_eff" = "Electrolyzer Efficiency (%)", "ship_cost" = "Ship Cost (€)", "elcz_cost" = "Electrolyzer Cost (€/MW)")  # Editable legend labels
  ) +  
  scale_linetype_manual(
    values = c("elc_cost" = "solid", "elcz_eff" = "dashed", "ship_cost" = "twodash", "elcz_cost" = "dotted"),
    labels = c("elc_cost" = "Electricity Cost (€/MWh)", "elcz_eff" = "Electrolyzer Efficiency (%)", "ship_cost" = "Ship Cost (€)", "elcz_cost" = "Electrolyzer Cost (€/MW)")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  theme(legend.key.width = unit(1.5, "cm"),
        legend.key.height = unit(0.4, "cm"),
        legend.text = element_text(size = 8),
