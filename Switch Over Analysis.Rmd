...
```{r}
undebug(Calc.Topologies)
Calc.Topologies(LAG = 0, PLANT = 1000)
```
...
```{r Figure 4}
# Figure4
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, LAG = 0)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 0)
...
text(2340,0.157,"14-year lag", cex = 1.2, srt = 45, adj = 1)
text(2470,0.089,"7-year lag", cex = 1.2, srt = 29, adj = 1)
text(2470,0.061,"Overnight deployment", cex = 1.2, srt = 21, adj = 1)
...
```{r Figure 5}
# Figure5
Result.high <- Calc.Topologies(EL.P = 35, PEM.P = 1926000, LAG = 14)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 7)
Result.low <- Calc.Topologies(EL.P = 29, PEM.P = 1198000, LAG = 0)

# Figure5. best for topology B
Result.high <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 14)
Result.mid <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 7)
Result.low <- Calc.Topologies(EL.P = 33, PEM.P = 1519000, LAG = 0)
...
Dist <- 100 #375 #km distance of the EEZ
...
H2_prices <- seq(0,10,by = 0.1) #prices in EUR/kgH2
EL_prices <- seq(0, 0.5, by = 0.005) #prices in EUR/kWh
...
for(HP in H2_prices_MJ){
  for(EP in EL_prices_MJ){
    ...
    TOPA_Profit <- EP - Mid_Topologies$TopologyA_EUR.MJ
...
Heatmap_Matrix <- as.matrix(sparseMatrix(i = Heatmap_DF$EL_Price, j = Heatmap_DF$H2_Price, x = Heatmap_DF$C_A_Difference))

Heatmap_Matrix <- select(Heatmap_DF, EL_Price, H2_Price, C_A_Difference) %>%
  pivot_wider(names_from = EL_Price, values_from = C_A_Difference)

Heatmap_Matrix2 <- as.matrix(Heatmap_Matrix, dimnames = "H2_Price")
