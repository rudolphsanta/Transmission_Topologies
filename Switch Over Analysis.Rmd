...
LCOE <- function(CAPEX, OPEX, ann.energy, r, life, lag = 0, type = 1) { #MEUR, MEUR/yr, MJ,  rate, lifetime 
  Inv <- CAPEX*1000000 #EUR
  Ann.V <- rep(1000000*OPEX,times = life) #EUR/yr
  if(lag == 0) { #if no lag
    CF <- c(Inv,Ann.V)
  } else {
    if(type == 1) {CF <- c(Inv, rep(0,times = lag), Ann.V)} #investment realized in year 0, and opex begins at start of life
    if(type == 2) {CF <- c(rep(Inv/(lag + 1), times = lag + 1), Ann.V)} #investment realized evenly over the lag period
...
Topology_A <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, r = 0.052, life = 25, curtailment = 0.05, interconnection = 3, CF.type = 2,ITC = 0) { #Follows Van Wingerden et al. (2023)
...
Topology_B <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, PEM.price = 1926000, PIPE.price = 615, r = 0.08, life = 25, lag = 3, CF.type = 2) { #Follows Van Wingerden et al. (2023)
...
Topology_C <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, PEM.price = 1926000, SHIP.price = 43000000, r = 0.08, life = 25, lag = 3, CF.type = 2) { 
...
Calc.Topologies <- function(PLANT = 1000, EL.P = 33, CF = 0.5, CURT = 0.05, PEM.P = 15519000, SHIP.P = 43000000, PIPE.P = 615, DISC = 0.052, LAG = 3, CF.T = 2) {
  ...
  for(dist in dist.V) {
    ...
    result.B <- Topology_B(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, PIPE.price = PIPE.P, r = DISC, lag = LAG, CF.type = CF.T)
    result.C <- Topology_C(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, SHIP.price = SHIP.P, r = DISC, lag = LAG, CF.type = CF.T)
...
# result.1 <- Topology_A(Dist.km, Plant.MW, curtailment = 0, interconnection = 5, CF.type = 1)
# result.2 <- Topology_A(Dist.km, Plant.MW, curtailment = 0, interconnection = 5, CF.type = 2)
# result.3 <- Topology_A(Dist.km, Plant.MW, curtailment = 0, interconnection = 0, CF.type = 3)
# result.4 <- Topology_A(Dist.km, Plant.MW, curtailment = 0, interconnection = 5, CF.type = 4) #need to do several runs of this
# 
# result.DF <- bind_rows(result.1,result.2,result.3,result.4)
```
...
plot(x = Result.A.3$distance_km, y = Result.A.3$TopologyA_EUR.kWh, type = 'b', pch = 19, col = 'orange', ylim = c(0,0.4), las = 1, xaxs = 'i', yaxs = 'i', xlab = "Distance from Shore [km]", ylab = "LCOE [EUR/kWh]")
polygon(x = c(Result.A.15$distance_km,rev(Result.A.15$distance_km)),
        y = c(Result.A.15$TopologyA_EUR.kWh,rev(Result.A.0$TopologyA_EUR.kWh)), 
...
```{r}
result <- replicate(10, Topology_A(20, 1000, curtailment = 0.05, interconnection = 0, CF.type = 3)$LCOE.tot) #calculate the LCOE n times
```
...
```{r VISUALIZE Lag}
plot(x = RES.2$lag.years, y = RES.2$LCOE, pch = 19, type = 'b', xlim = c(0,15), ylim = c(60,80),
...
polygon(x = c(2,6,6,2), y = c(0,0,200,200), col = rgb(255/255,128/255,0,0.3), border = FALSE) #typical construction time
polygon(x = c(5,12,12,5), y = c(0,0,200,200), col = rgb(0/255,153/255,0,0.3), border = FALSE) #typical deployment time
...
```
```{r SENSITIVITY ANALYSIS ON discount rate, years of lag, project size, distance to shore, electricity cost, ITC}
...
Baseline <- Topology_A(dist.km = 20, 
                       ...
                       curtailment = 0.05, 
                       interconnection = 10, 
                       CF.type = 2)$LCOE.tot #in EUR/MWh. 65.29
#Discount rate
low.r <- 0.02
high.r <- 0.1
...
Policy.ITC <- Topology_A(dist.km = 20, 
                         ...
                         interconnection = 10, 
                         CF.type = 2,
                         ITC = 0.10)$LCOE.tot #61.88 EUR/MWh 
...
Policy.LAG <- Topology_A(dist.km = 20, 
                         ...
                         interconnection = 9, 
                         CF.type = 2,
                         ITC = 0)$LCOE.tot #63.55 EUR/MWh. What will it cost to reduce permitting time by 1 year?
...
Result.high <- Calc.Topologies(LAG = 20, CURT = 0.20)
