...
Topology_A <- function(dist.km, plant.MW, cf = 0.5, EL.price = 35, r = 0.08, life = 25, curtailment = 0.05, interconnection = 3, CF.type = 2) { #Follows Van Wingerden et al. (2023)
...
```{r Results}
RES.1 <- Lag.Value(100, 1000, 1)
RES.2 <- Lag.Value(100, 1000, 2)
RES.3 <- Lag.Value(100, 1000, 3)
RES.4 <- Lag.Value.4(100, 1000, n = 1000)
```
...
Calc.Topologies <- function(PLANT = 2000, EL.P = 35, CF = 0.5, CURT = 0.05, PEM.P = 1926000, SHIP.P = 43000000, PIPE.P = 615, DISC = 0.08, LAG = 3) {
  ...
  for(dist in dist.V) {
    result.A <- Topology_A(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, r = DISC, curtailment = CURT, interconnection = LAG, CF.type = 2)
    result.B <- Topology_B(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, PIPE.price = PIPE.P, r = DISC, lag = LAG, CF.type = 2)
    result.C <- Topology_C(dist.km = dist, plant.MW = PLANT, cf = CF, EL.price = EL.P, PEM.price = PEM.P, SHIP.price = SHIP.P, r = DISC, lag = LAG, CF.type = 2)
...
Result.high <- Calc.Topologies(LAG = 25)
